...
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  // Handle error cases, file absence and unpraseable file are serious errors
  var data = fs.readFileSync('/etc/mjc/app.config', 'utf8');
  var config = JSON.parse(data);
  traverse(settings).forEach(function (x) {
    if (typeof x === 'string' || x instanceof String) this.update(must.render(x, config));
  });
    ...
    settings.operations.forEach(function(op) {
      ...
      if (functions) {
        for (var f in functions) {
          ...
          // If function value is a map with params key in it, use the params as list
          // Otherwise treat the function value as param list
          var params = functions[f].params ? functions[f].params : functions[f] ;
          var paramNames = [];
          var paramSources = [];
          ...
          // If function value is a map with path key use it, otherwise use the function name
          var path = functions[f].path || '/' + f;
          fn.accepts = [];
          fn.shared = true;
